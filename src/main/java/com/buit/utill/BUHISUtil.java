        ...
        try {
            ...
            for (int i = 0; i < dicsypcList.size(); i++) {
                ...
                if (dicsypcList.get(i).get("PCBM").toString().equals(as_sypc)) {
                    ...
                    //附加项目跟随药品计费,药品是发的第二天的药
                    if(parameters.get("FJBZ") != null && "1".equals(ObjectToTypes.parseString(parameters.get("FJBZ")))) {
                    	ldt_yljzsj = sdfdatetime.parse((sdfdate.format(getDateAfter(new Date(), 2)) + " 00:00:00"));
                    }
